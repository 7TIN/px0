...
| `web/src/tabs.js`      | `openFile()`, `closeTab()`, `switchTab()`, `drawTabs()`, `drawCrumbs()`. Image paths become lightweight `{ image: true }` tabs (no lines/gutter/LSP/outline). `reopenClosedTab()` (Alt+Shift+T) reopens from stack of last 20 closed files.                     |
